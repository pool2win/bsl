...
(struct utxo (block_height spent input-script output-script) #:mutable #:transparent)

;; A constant to define all coinbase outputs
(define coinbase null)

;; Define alice's coins earned in a coinbase output
;; TODO: The `(p2pkh ,alice:pubkey) consstruct can be (p2pkh
;; alice:pubkey) instead. Where p2pkh is macro returning the bitcoin
;; script.
(define alice-coins (utxo 0 #f coinbase `(p2pkh ,alice:pubkey)))
