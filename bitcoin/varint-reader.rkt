...
(define (read-varint-value in)
  (let* ([varint-2-bytes-flag (integer->integer-bytes #xfd 1 #f)]
         ...
         [value (cond
                  [(equal? first-byte varint-8-bytes-flag) (read-bytes 8 in)]
                  [(equal? first-byte varint-4-bytes-flag) (read-bytes 4 in)]
                  [(equal? first-byte varint-2-bytes-flag) (read-bytes 2 in)]
                  [else first-byte])])
